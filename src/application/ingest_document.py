...
class IngestDocumentUseCase:
    """Orchestrates document chunk ingestion into Neo4j and pgvector using shared chunk IDs."""
    ...
    def execute(self, document_id: str, text_chunks: List[str], section_paths: Optional[List[str]] = None) -> List[ChunkExtractionResult]:
        results = []
        ...
        for i, chunk_text in enumerate(text_chunks):
            ...
            # 3. Collect resolved entity canonical IDs
            resolved_entity_ids = [
                self.resolver.resolve(ent.name) for ent in extraction_result.entities
            ]

            # 4. Save into pgvector vector store
